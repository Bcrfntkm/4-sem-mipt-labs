...
public:
    ...
    CalcNode() : x(0.0), y(0.0), z(0.0), smth(4000.0), vx(0.0), vy(0.0), vz(0.0), wx(0.0), wy(0.0), wz(0.0), eps(0.0)
    {
    }

    // Конструктор с указанием всех параметров
    CalcNode(double x, double y, double z, double smth, double vx, double vy, double vz, double wx, double wy, double wz, double eps)
        : x(x), y(y), z(z), smth(smth), vx(vx), vy(vy), vz(vz), wx(wx), wy(wy), wz(wz), eps(eps)
    {
    }

    // Метод отвечает за перемещение точки
    // Движемся время tau из текущего положения с текущей скоростью
    void move(double tau, uint step) {
        if(step == 25){
            eps += -0.05;
        }
        if(step == 51){
    ...
    void update_smth(double t){
        smth = 4500 + 1000 * cos(M_PI * pow(pow(x, 2) + pow(z + 42, 2) , 0.5) / 50 - 100 * t) ;
    }
...
public:
    ...
    CalcMesh(const std::vector<double>& nodesCoords, const std::vector<std::size_t>& tetrsPoints) {
        ...
        for (unsigned int i = 0; i < nodesCoords.size() / 3; i++) {
            ...
            double smth = 4000;
            nodes[i] = CalcNode(pointX, pointY, pointZ, smth, 0.0, 0.0, 0.0, 0.0, 0.7, 0.0, -0.015);
        }
    ...
    void doTimeStep(double tau, uint step) {
    ...
    void snapshot(unsigned int snap_number) {
        ...
        string fileName = "test" + std::to_string(snap_number) + ".vtu";
